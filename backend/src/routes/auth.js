...
router.put('/profile', auth, async (req, res) => {
  try {
    const allowed = ['name', 'goal', 'gender', 'age', 'height', 'weight', 'activity_level',
    ...
    const u = rows[0];
    res.json({
      id: u.id, email: u.email, name: u.name, goal: u.goal, gender: u.gender,
      age: u.age, height: u.height, weight: u.weight, activity_level: u.activity_level,
      daily_calories: u.daily_calories, daily_proteins: u.daily_proteins,
      daily_fats: u.daily_fats, daily_carbs: u.daily_carbs,
      is_premium: !!u.is_premium, is_onboarded: !!u.is_onboarded,
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Ошибка сервера' });
